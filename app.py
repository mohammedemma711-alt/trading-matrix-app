...
class MasterTradingEngine:
    ...
    def analyze_market(df, style="Day Trade"):
        ...
        # =====================================================================
        # 📐 INTEGRATED AUTOMATED FIBONACCI GRID CALCULATOR
        # =====================================================================
...
# =====================================================================
# 📊 LAYER 3: WEB APP INTERFACE LAYOUT WITH HISTORICAL LOG ENGINE
# =====================================================================
...
# Core State Initialization Engine
...
# Helper closure function to trace running lifecycle results of active trades
def update_trade_history_status(current_asset, latest_close_price):
    for trade in st.session_state.trade_history:
        if trade["Asset"] == current_asset and trade["Status"] == "ON TRADE":
            if trade["Type"] == "BUY / LONG":
                if latest_close_price >= trade["Take Profit"]:
                    trade["Status"] = "HIT TP 🟢"
                elif latest_close_price <= trade["Stop Loss"]:
                    trade["Status"] = "HIT SL 🔴"
...
if scan_button or 'first_run' not in st.session_state:
    ...
    if df_chart is not None:
        ...
        # Run history check to see if older entries for this specific asset need updating
        update_trade_history_status(target_pair, current_price)

        # Helper string conversion variables to eliminate potential formatting TypeErrors
        poi_str = f"${predicted_poi:,.2f}" if predicted_poi is not None else "Calculating..."
        ob_str = f"${metrics['order_block']:,.2f}" if metrics['order_block'] is not None else "NONE"
        bb_str = f"${metrics['breaker_block']:,.2f}" if metrics['breaker_block'] is not None else "NONE"

        # =====================================================================
        # 🚨 THE PREDICATIVE "SHOW AND TELL" RADAR CARD
        # =====================================================================
        st.subheader("🔮 Impending Point of Interest (POI) Radar")
        if predicted_poi is not None:
            distance = abs(current_price - predicted_poi)
            pct_distance = (distance / current_price) * 100
            
            p_col1, p_col2, p_col3 = st.columns([1.5, 1, 2.5])
            p_col1.metric("PREDICTED POI TARGET ZONE", poi_str, help="The system calculated this macro boundary line ahead of time.")
        ...
        if len(all_signals) >= 2 and master_bias != "NEUTRAL" and metrics["candle_pattern"] != "NONE":
            ...
            # Check duplication before recording into structural session dictionary array
            is_duplicate = any(
                t["Asset"] == target_pair and 
                abs(t["Entry Price"] - entry_price) < 0.0001 and 
                t["Type"] == trade_type 
            ...
            if not is_duplicate:
                new_trade_record = {
                    "Date Given": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "Asset": target_pair,
                    "Horizon": style_label,
                    "Type": trade_type,
                ...
                st.session_state.trade_history.append(new_trade_record)
            ...
            exec_col1.metric("POINT OF INTEREST (POI)", poi_str)
            ...
            st.markdown(f"**Current Status:** The asset is tracking toward the calculated POI zone of **{poi_str}**. No entry values will print under this block until the market successfully reaches this zone and registers an official confirmation candle signature.")
        ...
        with check_col2:
            ...
            st.markdown(f"• **Active Order Block Zone:** `{ob_str}`")
            st.markdown(f"• **Active Breaker Block Level:** `{bb_str}`")
...
# =====================================================================
# 📜 NEW FEATURE: PERSISTENT STRATEGY HISTORY ZONE
# =====================================================================
st.divider()
st.subheader("📜 System Position Execution & History Zone")

if st.session_state.trade_history:
    # Build dataframe log directly from structural browser memory array tracking
    history_df = pd.DataFrame(st.session_state.trade_history)
    
    # Re-order table architecture representation layout seamlessly
    history_df = history_df[["Date Given", "Asset", "Horizon", "Type", "Entry Price", "Stop Loss", "Take Profit", "Status"]]
    
    # Render interactive DataFrame visualization
    st.dataframe(history_df, use_container_width=True, hide_index=True)
    
    if st.button("🧹 Clear History Log Engine"):
        st.session_state.trade_history = []
        st.rerun()
else:
    st.info("No trades have entered structural entry matrix gates yet. History records will append here when conditions trigger an entry.")
